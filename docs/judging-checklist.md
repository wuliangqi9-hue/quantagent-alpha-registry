# Judging Checklist

Use this checklist before recording the demo video and submitting to DoraHacks.

## Value Proposition

- The one-line pitch is visible and clear.
- The project explains why AI plus on-chain proof matters.
- The project does not overclaim guaranteed returns.
- The target users are clear: traders, researchers, and Web3 protocols.

## Technical Depth

- Factor engine is real, not hardcoded prose.
- Strategy selector uses existing benchmark evidence.
- The app can run in live mode and offline demo mode.
- The signal hash links the off-chain decision report to the on-chain record.
- The Mantle transaction or contract is visible from the UI.

## Mantle Contribution

- The proof layer is deployed or demonstrated on Mantle.
- The README explains why Mantle is part of the system, not decoration.
- The transaction/event trail is easy for judges to inspect.

## Product Completeness

- Public frontend works without localhost.
- Submitted demo URL does not contain `localhost`, `127.0.0.1`, or a private LAN address.
- Demo flow finishes in under 3 minutes.
- No broken buttons or empty states in the main path.
- Charts are readable on a laptop screen.
- The UI has a fallback state when live APIs fail.

## Research Credibility

- Factor list is concise and explainable.
- Benchmarks are shown with caveats.
- Limitations are explicit.
- Existing QuantAgent/Hummingbot work is referenced as prior evidence.

## Submission Assets

- Open-source repo link.
- Public app URL.
- Contract address or Mantle explorer transaction.
- Demo video.
- README with setup instructions.
- Architecture diagram.
- Short pitch.
